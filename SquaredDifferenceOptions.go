// Code generated by the FlatBuffers compiler. DO NOT EDIT.

package tflite

import (
	flatbuffers "github.com/google/flatbuffers/go"
)

type SquaredDifferenceOptionsT struct {
}

func (t *SquaredDifferenceOptionsT) Pack(builder *flatbuffers.Builder) flatbuffers.UOffsetT {
	if t == nil { return 0 }
	SquaredDifferenceOptionsStart(builder)
	return SquaredDifferenceOptionsEnd(builder)
}

func (rcv *SquaredDifferenceOptions) UnPackTo(t *SquaredDifferenceOptionsT) {
}

func (rcv *SquaredDifferenceOptions) UnPack() *SquaredDifferenceOptionsT {
	if rcv == nil { return nil }
	t := &SquaredDifferenceOptionsT{}
	rcv.UnPackTo(t)
	return t
}

type SquaredDifferenceOptions struct {
	_tab flatbuffers.Table
}

func GetRootAsSquaredDifferenceOptions(buf []byte, offset flatbuffers.UOffsetT) *SquaredDifferenceOptions {
	n := flatbuffers.GetUOffsetT(buf[offset:])
	x := &SquaredDifferenceOptions{}
	x.Init(buf, n+offset)
	return x
}

func (rcv *SquaredDifferenceOptions) Init(buf []byte, i flatbuffers.UOffsetT) {
	rcv._tab.Bytes = buf
	rcv._tab.Pos = i
}

func (rcv *SquaredDifferenceOptions) Table() flatbuffers.Table {
	return rcv._tab
}

func SquaredDifferenceOptionsStart(builder *flatbuffers.Builder) {
	builder.StartObject(0)
}
func SquaredDifferenceOptionsEnd(builder *flatbuffers.Builder) flatbuffers.UOffsetT {
	return builder.EndObject()
}
