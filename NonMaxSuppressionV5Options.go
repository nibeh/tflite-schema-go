// Code generated by the FlatBuffers compiler. DO NOT EDIT.

package tflite

import (
	flatbuffers "github.com/google/flatbuffers/go"
)

type NonMaxSuppressionV5OptionsT struct {
}

func (t *NonMaxSuppressionV5OptionsT) Pack(builder *flatbuffers.Builder) flatbuffers.UOffsetT {
	if t == nil { return 0 }
	NonMaxSuppressionV5OptionsStart(builder)
	return NonMaxSuppressionV5OptionsEnd(builder)
}

func (rcv *NonMaxSuppressionV5Options) UnPackTo(t *NonMaxSuppressionV5OptionsT) {
}

func (rcv *NonMaxSuppressionV5Options) UnPack() *NonMaxSuppressionV5OptionsT {
	if rcv == nil { return nil }
	t := &NonMaxSuppressionV5OptionsT{}
	rcv.UnPackTo(t)
	return t
}

type NonMaxSuppressionV5Options struct {
	_tab flatbuffers.Table
}

func GetRootAsNonMaxSuppressionV5Options(buf []byte, offset flatbuffers.UOffsetT) *NonMaxSuppressionV5Options {
	n := flatbuffers.GetUOffsetT(buf[offset:])
	x := &NonMaxSuppressionV5Options{}
	x.Init(buf, n+offset)
	return x
}

func (rcv *NonMaxSuppressionV5Options) Init(buf []byte, i flatbuffers.UOffsetT) {
	rcv._tab.Bytes = buf
	rcv._tab.Pos = i
}

func (rcv *NonMaxSuppressionV5Options) Table() flatbuffers.Table {
	return rcv._tab
}

func NonMaxSuppressionV5OptionsStart(builder *flatbuffers.Builder) {
	builder.StartObject(0)
}
func NonMaxSuppressionV5OptionsEnd(builder *flatbuffers.Builder) flatbuffers.UOffsetT {
	return builder.EndObject()
}
